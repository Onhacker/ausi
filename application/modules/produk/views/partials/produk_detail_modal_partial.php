...
<style>
  ...
  /* spinner style dari sistem kamu */
    ...
    <div class="row align-items-center no-gutters mb-2">
      ...
      <div class="col-4">
        <div class="meta-line mb-0">
          <?php if ($stok > 0): ?>
            <span class="chip" title="Stok tersedia">
              <span class="dot on"></span> Instock
            </span>
          <?php else: ?>
            <span class="chip" title="Stok habis">
              <span class="dot off"></span> Sold Out
            </span>
          <?php endif; ?>
        </div>
      </div>
      ...
      <div class="col-4 text-right">
        <button type="button"
          ...
          <!-- text -->
          <span class="btn-text d-none d-sm-inline">
            Tambah ke Keranjang
          </span>
        </button>
      </div>
...
(function(){
  ...
  // Tekan Enter di qty => trigger klik tombol add cart
  input.addEventListener('keydown', function(e){
    if (e.key === 'Enter'){
      e.preventDefault();
      const $btn = $('#btn-add-cart-modal');
      if ($btn.length && !$btn.hasClass('btn-loading') && !$btn.is(':disabled')) {
        $btn.click();
      }
    }
  });

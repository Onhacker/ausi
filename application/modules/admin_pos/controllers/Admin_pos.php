...
public function rawbt_payload($id)
{
    // ======= Ambil data transaksi (SAMAKAN dengan yang di view) =======
    $order = $this->OrderModel->find($id);           // sesuaikan
    $items = $this->OrderModel->items($id);          // sesuaikan
    $store = $this->StoreModel->getActiveStore();    // sesuaikan
    ...
    $paper    = $this->input->get('paper') ?: '80';
    $cols     = (int)($this->input->get('cols') ?: (($paper==='80') ? 48 : 32));
    $cutn     = (int)($this->input->get('cutn') ?: 7);
    $trail    = (int)($this->input->get('trail') ?: 3);
    $cutmode  = strtolower($this->input->get('cutmode') ?: 'partial'); // partial|full
    ...
    $lineLR = function($left, $right) use($cols){
    ...
    $wrap = function($s, $w) {
        $s = (string)$s; $out=[]; while (strlen($s) > $w) { $out[] = substr($s, 0, $w); $s = substr($s, $w); }
        if ($s !== '') $out[] = $s; return $out;
    };

    $fmtRp = fn($n) => 'Rp '.number_format((int)$n, 0, ',', '.');

    // ======= (Opsional) Cetak logo dengan ESC * 24-dot =======
    $logoBin = '';
    $logoPath = FCPATH.'assets/images/logo_admin.png'; // ganti ke asetmu
    if (is_file($logoPath) && function_exists('imagecreatefromstring')) {
        $logoBin = $this->_escposImage24dot($logoPath, ($paper==='80') ? 576 : 384);
    }

    // ======= Bangun payload ESC/POS =======
    $out  = $INIT . $SIZE1;

    // Logo
    if ($logoBin !== '') {
        $out .= $CTR . $logoBin . $LEFT;
    }

    // Header toko
    $out .= $CTR . $store->nama . "\n";
    foreach ($wrap($store->alamat ?? '', $cols) as $ln) { if($ln!=='') $out .= $CTR.$ln."\n"; }
    if (!empty($store->telp)) $out .= $CTR.'HP/WA: '.$store->telp."\n";
    $out .= $LEFT;
    ...
    $modeLabel = ($modeRaw==='dinein' || $modeRaw==='dine-in') ? 'Makan di tempat / Dine in'
               : (($modeRaw==='delivery') ? 'Antar / Kirim / Delivery' : 'Bungkus / Take Away');

    $out .= 'No: '.$nomor."\n";
    $out .= 'Waktu: '.$waktu."\n";
    $out .= 'Mode: '.$modeLabel."\n";
    if (($modeRaw==='dinein' || $modeRaw==='dine-in') && !empty($order->meja_nama)) {
        $out .= 'Meja: '.$order->meja_nama."\n";
    }
    if (!empty($order->nama)) { $out .= 'Pelanggan: '.$order->nama."\n"; }

    // Tabel Items (layout 80mm/58mm)
    $W_NAME = ($cols===48)?22:16;
    $W_QTY  = ($cols===48)?5:4;
    $W_PRC  = ($cols===48)?10:6;
    $W_SUB  = $cols - $W_NAME - $W_QTY - $W_PRC;

    $padR = fn($s,$n)=> (strlen($s)>= $n) ? substr($s,0,$n) : $s . str_repeat(' ', $n-strlen($s));
    $padL = fn($s,$n)=> (strlen($s)>= $n) ? substr($s,-$n) : str_repeat(' ', $n-strlen($s)) . $s;

    $rowHeader = function() use($padR,$padL,$W_NAME,$W_QTY,$W_PRC,$W_SUB){
        return $padR('Item',$W_NAME).$padR('Qty',$W_QTY).$padL('Harga',$W_PRC).$padL('Subtotal',$W_SUB)."\n";
    };
    $rowItem = function($name,$qty,$harga,$sub) use($padR,$padL,$W_NAME,$W_QTY,$W_PRC,$W_SUB){
        return $padR($name,$W_NAME).$padL($qty,$W_QTY).$padL($harga,$W_PRC).$padL($sub,$W_SUB)."\n";
    };
    ...
    foreach ($items as $idx => $it) {
        ...
        $qty = (int)($it->qty ?? 0);
        $hrg = (int)($it->harga ?? 0);
        $sub = (int)($it->subtotal ?? ($qty*$hrg));
        $total += $sub;

        $lines = $wrap($nm, $W_NAME);
        $out .= $rowItem(array_shift($lines) ?: '', (string)$qty, $fmtRp($hrg), $fmtRp($sub));
        foreach ($lines as $ln) { $out .= $rowItem($ln, '', '', ''); }
        if ($idx < (count($items)-1)) $out .= $HR1;
    }

    // Totals
    $out .= $HR1;
    $statusRaw = strtolower($order->status ?? '');
    $statusLbl = ($statusRaw==='paid') ? 'Lunas' : (($statusRaw==='verifikasi') ? 'Verifikasi' : 'Menunggu pembayaran');
    $out .= $lineLR('Status Pembayaran', $statusLbl)."\n";

    $kodeUnik = (int)($order->kode_unik ?? 0);
    $paidMethod = strtolower($order->paid_method ?? '');
    $showKodeUnik = ($paidMethod !== 'cash' && $kodeUnik > 0);

    if ($showKodeUnik) $out .= $lineLR('Kode Unik', $fmtRp($kodeUnik))."\n";
    $out .= $lineLR('Total', $fmtRp($total))."\n";

    $deliveryFee = (int)($order->delivery_fee ?? 0);
    $isDelivery  = ($modeRaw === 'delivery');
    if ($isDelivery && $deliveryFee>0) $out .= $lineLR('Ongkir', $fmtRp($deliveryFee))."\n";

    $grandTotal = isset($order->grand_total) ? (int)$order->grand_total : ($total + ($isDelivery?$deliveryFee:0) + $kodeUnik);
    $out .= $lineLR('Total Pembayaran', $fmtRp($grandTotal))."\n";

    // Footer
    if (!empty($store->footer)) $out .= "\n".$CTR.$store->footer."\n".$LEFT;
    $out .= $CTR.'Dicetak: '.date('d/m/Y H:i')."\n".$LEFT;
    $out .= $CTR.'Dev By Onhacker'."\n".$LEFT;

    // Feed & Cut (kalibrasi)
    $out .= str_repeat("\n", max(0,$trail));
    if ($cutmode === 'full') {
        $out .= $GS.'V'."\x42".chr($cutn); // full cut with feed=n
    } else {
        $out .= $GS.'V'."\x41".chr($cutn); // partial cut with feed=n
    }

    // ======= Output sebagai RAW BYTES =======
    header('Content-Type: application/octet-stream');
    header('Cache-Control: no-store');
    header('Content-Disposition: inline; filename="rawbt-'.$id.'.bin"');
    // Penting: JANGAN kirim UTF-8/JSON. Ini murni biner.
    echo $out;
    exit;
}

/**
 * Konversi gambar ke ESC * (24-dot, double-density) baris-per-baris.
 * $maxDots: lebar maksimum print head (80mm≈576, 58mm≈384).
 */
private function _escposImage24dot(string $path, int $maxDots): string
{
    $bin = '';
    $data = @file_get_contents($path);
    if ($data === false) return $bin;
    $im = @imagecreatefromstring($data);
    if (!$im) return $bin;

    $w = imagesx($im); $h = imagesy($im);
    $scale = min(1.0, $maxDots / max(1,$w));
    $nw = max(8, (int)floor($w * $scale));
    ...
    for ($y=0; $y<$nh; $y+=24) {
        ...
        $bin .= "\x1B".'*' . chr(33) . chr($bytesPerRow & 0xFF) . chr(($bytesPerRow>>8) & 0xFF);
        for ($x=0; $x<$nw; $x++) {
            for ($k=0; $k<3; $k++) {
                $byte = 0;
                for ($b=0; $b<8; $b++) {
                    ...
                    $bit = 0;
                    if ($yy < $y + $sliceH) {
                        $rgb = imagecolorat($dst, $x, $yy);
                        $r = ($rgb >> 16) & 0xFF;
                        $g = ($rgb >> 8 ) & 0xFF;
                        $bl= ($rgb      ) & 0xFF;
                        $lum = 0.299*$r + 0.587*$g + 0.114*$bl;
                        $bit = ($lum < 160) ? 1 : 0; // threshold
                    }
                    $byte |= ($bit << (7 - $b));
                }
                $bin .= chr($byte);

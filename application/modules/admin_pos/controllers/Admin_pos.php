...
$fmt_method = function($raw, $voucher_code = null, $voucher_disc = 0, $order_status = '') {
    ...
    if ($voucher_disc > 0 || $codeTrim !== '') {
        ...
        $chips[] =
            '<span class="badge badge-pill voucher-chip mr-1">'
        ...
        foreach ($list as $r) {
           ...
           // Meja + nama
            ...
            if (!$isKitchen && !$isBar) {
                ...
                $btnPaid   = '<button type="button" class="btn btn-xs btn-primary mr-1" onclick="mark_paid_one('.$idInt.')"><i class="fe-dollar-sign"></i></button>';
                $btnCancel = '<button type="button" class="btn btn-xs btn-secondary mr-1" onclick="mark_canceled_one('.$idInt.')" '.($isClosed ? 'disabled' : '').'><i class="fe-shield-off"></i></button>';

                $unameLower = strtolower((string)$this->session->userdata('admin_username'));
                $isAdmin    = ($unameLower === 'admin');
                $btnDelete  = $isAdmin ? '<button type="button" class="btn btn-xs btn-danger" onclick="hapus_data_one('.$idInt.')"><i class="fa fa-trash"></i></button>' : '';
...
{
  $row = $this->db->select('gmail_token')->get_where('settings', ['id'=>1])->row();
...
/**
 * Satu pintu: siap pakai (auto-refresh + merge aman)
 */
private function _gmail_service_ready(): \Google\Service\Gmail
{
  ...
  if ($client->isAccessTokenExpired()) {
    ...
    // merge aman: token lama + token baru (refresh_token jangan ketimpa kosong)
    $merged = $tok;
...
private function _gmail_sync(int $limit = 20, int $maxToProcess = 200, int $bootstrapDays = 7): array
{
  try {
    $svc = $this->_gmail_service_ready();
    ...
    if (!empty($lastSyncOld)) {
      ...
      $dt->modify('-1 day'); // safety window
      $q = 'after:'.$dt->format('Y/m/d');
    } else {
      $q = 'newer_than:'.max(1,$bootstrapDays).'d';
    }

    $pageSize     = max(1, min(500, $limit));          // ukuran per request
    $maxToProcess = max($pageSize, (int)$maxToProcess); // total maksimal diproses per sync
    ...
    while (true) {
      $remaining = $maxToProcess - $fetchedTotal;
      ...
      $fetchedTotal += count($msgs);

      // prefetch existing gmail_id
      ...
      foreach ($msgs as $m) {
        ...
        if ($ok) {
          ...
          if ((int)($err['code'] ?? 0) === 1062) {
            // duplicate key (uq_gmail_id / uq_trx_ref) -> anggap skip
            continue;
          }
          $dbFail++;
        }
      ...
      $nextToken = (string)($list->getNextPageToken() ?? '');
      if ($nextToken === '') {
        $nextToken = null;
        break; // paging selesai
      }
    }

    // ✅ update last sync hanya kalau paging selesai (tidak ada nextToken & tidak dipotong maxToProcess)
    $lastSyncNew = $lastSyncOld;
    if (!$hasMore && $maxInternalMs > 0) {
      $lastSyncNew = date('Y-m-d H:i:s', (int)($maxInternalMs/1000));
      if ($lastSyncNew !== $lastSyncOld) {
    ...
    return [
      ...
      'has_more' => $hasMore,           // ✅ kalau true, jalankan sync lagi

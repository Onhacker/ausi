...
class Admin_ps extends Admin_Controller {

    public function __construct(){
        parent::__construct();
        $this->load->model('M_admin_ps','dm');
        cek_session_akses(get_class($this), $this->session->userdata('admin_session'));
    }

    public function index(){
        $data["controller"] = get_class($this);
        $data["title"]      = "Transaksi Sewa PlayStation";
    ...
    private function _get_setting(){
        ...
        if(!$row){
            ...
            return (object)[
                'harga_satuan'        => 15000, // default PS
                'durasi_unit'         => 60,    // 60 menit per sesi
    ...
    /**
     * Tambah poin voucher PS: 1 baris di table voucher_ps = 1x main.
 ...
 */
    private function _insert_voucher_rows($idTransaksi, $hp, $nama, $sesi, $status){
        ...
        // Cek pengaturan: kalau threshold <= 0, program voucher PS NONAKTIF
        $setting   = $this->_get_setting();
        $threshold = (int)($setting->free_main_threshold ?? 0);
        if ($threshold <= 0) {
            // Tidak menggunakan voucher: tidak usah catat poin sama sekali
            return;
        }

        $now = date('Y-m-d H:i:s');
        for ($i = 0; $i < $sesi; $i++) {
            $this->db->insert('voucher_ps',[
    ...
    /**
     * Hapus poin voucher PS yang sumbernya dari transaksi tertentu
    ...
    // NOTE:
    // - harga_satuan  : boleh 0 ke atas (tapi biasanya > 0)
    // - durasi_unit   : wajib > 0
    // - freeMain      : boleh 0 (0 = TIDAK menggunakan voucher)
        if ($harga < 0 || $durasi <= 0 || $freeMain < 0) {
            echo json_encode([
                "success"=>false,
                "title"  =>"Gagal",
                "pesan"  =>"Input tidak valid. Durasi harus > 0 dan jumlah main FREE minimal 0."
            ]);
            return;
        }
        ...
        $ok  = $row
            ? $this->db->update('ps_setting',$dataUpdate,['id'=>1])
            : $this->db->insert('ps_setting',array_merge(['id'=>1],$dataUpdate));

        echo json_encode(
            $ok
            ? ["success"=>true,"title"=>"Berhasil","pesan"=>"Pengaturan tersimpan"]
    ...
    public function get_data(){
        ...
        foreach ($list as $r) {
            ...
            $row = [
                ...
                'aksi'     => $btnEdit.$btnBayar.$btnBatal, // btnHapus pakai bulk
            ];
            $data[] = $row;
        }
    ...
    public function get_one($id){
        $id  = (int)$id;
        $row = $this->db->get_where('ps_transaksi',['id_transaksi'=>$id])->row();
        if(!$row){
    ...
    public function set_batal(){
        ...
        $ok = $this->db->update('ps_transaksi', ['status'=>'batal'], ['id_transaksi'=>$id]);
    ...
    public function update(){
        ...
        $ok = $this->db->update('ps_transaksi',$upd, ['id_transaksi'=>$id]);
    ...
    public function set_lunas(){
        ...
        $ok = $this->db->update('ps_transaksi',['status'=>'selesai'],['id_transaksi'=>$id]);

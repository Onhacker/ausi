...
<div class="container-fluid">
  <div class="hero-title">
    <h1 class="text mb-1">Pembayaran via Transfer</h1>
      <div class="text-white-50">Transfer sesuai nominal total di bawah ya, biar kasir gampang ngecek.</div>
...
<?php
  ...
  // status & token (untuk polling)
  $status_now = strtolower($order->status ?? 'pending');
  $token_poll = (string)($order->access_token ?? '');
?>
    ...
    <div class="col-md-6">
      <div class="card card-body mb-3">
        <div class="d-flex align-items-center justify-content-between mb-2">
          <h4 class="mb-0">Nominal</h4>
          <?php if (!in_array($status_now, ['paid','canceled','terkonfirmasi'], true)): ?>
...
(function(){
  ...
  const CART_URL = "<?= site_url('billiard/cart?t=') . urlencode($token_poll) ?>";
  ...
  function tick(){
    // baca deadline terbaru (disinkronkan oleh polling)
  ...
  tick();
    ...
    <!-- Kolom kanan: Ringkasan pesanan (tanpa item list) -->
    <div class="col-md-6">
      <div class="card card-body mb-3">
        ...
        <div class="soft mt-3">
          <div class="mini-hint">
            Setelah transfer, kasir akan memverifikasi pembayaran kamu. Kamu bisa kembali ke halaman
            <a href="<?= site_url('billiard/cart?t='.rawurlencode($order->access_token ?? '')) ?>">ringkasan booking</a>
...
<?php if (!empty($token_poll) && !in_array($status_now, ['terkonfirmasi','batal'], true)): ?>

<script>
(function(){
  var TOKEN   = "<?= htmlspecialchars($token_poll, ENT_QUOTES) ?>";
  var POLL_MS = 7000;
  ...
  async function check(){
    ...
    try{
      ...
      // countdown server habis → redirect cart (UX)

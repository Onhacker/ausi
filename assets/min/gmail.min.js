(function (w, $) {
  ...
  $('#gmail-page-info').text(curPage + '/' + totalPages);
  $('#gmail-prev-page').prop('disabled', curPage <= 1);
  $('#gmail-next-page').prop('disabled', curPage >= totalPages);
}
var curPage = 1;
var totalPages = 1;
var lastListXhr = null;

function loadInbox(opts) {
  opts = opts || {};
  var q = ($('#gmail-q').val() || '').trim();
  var page = opts.page || curPage || 1;

  if (!opts.silent) uiState('loading');

  // abort request sebelumnya (biar gak numpuk pas ngetik)
  if (lastListXhr && lastListXhr.readyState !== 4) {
    try { lastListXhr.abort(); } catch(e){}
  }

  lastListXhr = $.ajax({
    url: URL_LIST,
    dataType: 'json',
    data: { limit: opts.limit || 20, q: q, page: page },
    timeout: 15000
  });
  ...
  return lastListXhr
    .done(function (res) {
      ...
      if (!ok) { renderInbox([]); return; }

      curPage = (res.meta && res.meta.page) ? parseInt(res.meta.page,10) : page;
      totalPages = (res.meta && res.meta.pages) ? parseInt(res.meta.pages,10) : 1;
...
openInbox();
  ...
  .on('input.gmail', '#gmail-q', function () {
    ...
    t = setTimeout(function () {
      curPage = 1;
      loadInbox({ silent: false, limit: 20, page: 1 });
    }, 250);
  });
...
$(document).off('click.gmail', '#gmail-prev-page')
  .on('click.gmail', '#gmail-prev-page', function(){
    if (curPage > 1) loadInbox({ silent:false, limit:20, page: curPage-1 });
  });

$(document).off('click.gmail', '#gmail-next-page')
  .on('click.gmail', '#gmail-next-page', function(){
    if (curPage < totalPages) loadInbox({ silent:false, limit:20, page: curPage+1 });
  });

...
function scrollToGridProducts({offset=0, smooth=true} = {}){
 ...
 function serializeFilters(page=1){
  ...
  .done(function(r){
    ...
    const status = jq && jq.status ? jq.status : 0;

    // === KHUSUS 503 / 504 → tampilkan tombol reload ===
    if (status === 503 || status === 504) {
      $grid.html(
        '<div class="col-12 text-center">' +
          '<div class="alert alert-warning mb-3">Server lagi sibuk (Error '+status+').</div>' +
          '<button type="button" class="btn btn-outline-primary btn-sm js-products-reload">' +
            '<span class="btn-label mr-1"><i class="fe-refresh-ccw"></i></span>' +
            '<span class="btn-text">Muat ulang produk</span>' +
          '</button>' +
        '</div>'
      );
      $pagi.empty();

      // klik tombol → panggil ulang AJAX di page yang sama
      $grid
        .off("click", ".js-products-reload")
        .on("click", ".js-products-reload", function(e){
          e.preventDefault();
          loadProducts(page);  // pakai argumen page dari closure loadProducts
        });
